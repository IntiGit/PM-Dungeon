...
public class OnCollectAddToBag implements IOnCollect{
    ...
    public void onCollect(Entity worldItem, Entity whoCollected) {
        ...
        if(data.getItemName().equals("Rostige Klinge") || data.getItemName().equals("Jade Klinge")) {
            typetocheck = Waffe.class;
        } else if(data.getItemName().equals("Wasser") || data.getItemName().equals("Lila Gebräu")) {
            typetocheck = Trank.class;
        } else if(data.getItemName().equals("Dicke Treter") || data.getItemName().equals("Noch dickere Treter")) {
                ...
                hero -> {
                    if (whoCollected.equals(hero)) {
                        hero.getComponent(InventoryComponent.class)
                            .ifPresent(
                                (x) -> {
                                    if(addToBag( (InventoryComponent) x , typetocheck ,worldItem)) {
                                        Game.removeEntity(worldItem);
                                    } else if (((InventoryComponent) x).addItem(data)) {
                                        Game.removeEntity(worldItem);
                                    }
                                });
                    }
